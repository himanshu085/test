pipeline {
    ...
    stages {
        ...
        stage('Generate Coverage Report') {
            steps {
                dir('frontend') {
                    sh '''
                        npm test -- --coverage --coverageReporters=html --coverageDirectory=coverage --watchAll=false || true
                    '''
                }
            }
        }
    ...
    post {
        ...
        always {
            ...
            // deleteDir() is intentionally removed to preserve workspace
        }
    }
}
